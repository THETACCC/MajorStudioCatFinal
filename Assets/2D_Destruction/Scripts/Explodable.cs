...
public class Explodable : MonoBehaviour
{
    ...
    */
    public void explode(Vector2 collisionPoint, float collisionVelocity)
    {
        // If fragments were not created before runtime, generate them now
        if (fragments.Count == 0 && allowRuntimeFragmentation)
        {
            generateFragments();
        }
        ...
        else
        {
            foreach (GameObject frag in fragments)
            {
                ...
                // Add Rigidbody2D to the fragment if it doesn't already have one
                Rigidbody2D fragRigidbody = frag.GetComponent<Rigidbody2D>();
                if (fragRigidbody == null)
                {
                    fragRigidbody = frag.AddComponent<Rigidbody2D>();
                }

                // Calculate a force direction based on the fragment's position relative to the collision point
                Vector2 forceDirection = (frag.transform.position - (Vector3)collisionPoint).normalized;

                // Apply force based on the collision velocity and direction
                fragRigidbody.AddForce(forceDirection * collisionVelocity, ForceMode2D.Impulse);
            }
